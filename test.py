...
def fuse_test(model, vi_path, ir_path, output_path_root, index, use_sam):
    ...
    # Save result
    file_name = f'semfuse_{"withSAM" if use_sam else "noSAM"}_{str(index)}.png'
...
def main():
    ...
    with torch.no_grad():
        ...
        for i in range(opt.num_tests):
            ...
            # Fuse images
            start = time.time()
            fuse_test(model, visible_path, infrared_path, opt.output_path, index, opt.use_sam)
            end = time.time()
